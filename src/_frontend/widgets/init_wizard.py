...
class InitWizard(QWizard, Ui_Wizard, SignalsMixin):
    ...
    def __attrs_post_init__(self):
        self.alert.connect(qtinter.asyncslot(self.alert_manager))
        self.wizardLocalConfig_filedialog.clicked.connect(self.music_collection_select)
    ...
    async def alert_manager(self, page: AlertPage, style: AlertStyle, text: str):
        ...
        match page:
            case AlertPage.wizardStart:
                await self.alert_trigger(
                    self.wizardStart_alert, self.wizardStart_alert_text, style, text
                )
            case AlertPage.wizardServerConfig:
                await self.alert_trigger(
                    self.wizardServerConfig_alert,
            ...
            case AlertPage.wizardLocalConfig:
                await self.alert_trigger(
                    self.wizardLocalConfig_alert,
                    self.wizardLocalConfig_alert_text,
                    style,
                    text,
                )

    async def alert_trigger(
        self,
        alert: QWidget,
        alert_text: QLabel,
    ...
    def music_collection_select(self):
        ...
        d = self.file_dialog.getExistingDirectory(self, "Select music collection root")
